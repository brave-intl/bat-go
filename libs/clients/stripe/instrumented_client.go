...
var clientDurationSummaryVec = promauto.NewSummaryVec(
	prometheus.SummaryOpts{
		Name:       "stripe_client_duration_seconds",
...
func (_d ClientWithPrometheus) CreateOnrampSession(ctx context.Context, integrationMode string, walletAddress string, sourceCurrency string, sourceExchangeAmount string, destinationNetwork string, destinationCurrency string, supportedDestinationNetworks []string) (op1 *OnrampSessionResponse, err error) {
	...
	defer func() {
		...
		clientDurationSummaryVec.WithLabelValues(_d.instanceName, "CreateOnrampSession", result).Observe(time.Since(_since).Seconds())
	}()
	return _d.base.CreateOnrampSession(ctx, integrationMode, walletAddress, sourceCurrency, sourceExchangeAmount, destinationNetwork, destinationCurrency, supportedDestinationNetworks)
}
