...
//go:generate gowrap gen -p github.com/brave-intl/bat-go/payment -i Datastore -t ../.prom-gowrap.tmpl -o instrumented_datastore.go -l ""
...
// UpdateTransactionStatus implements Datastore
func (_d DatastoreWithPrometheus) UpdateTransactionStatus(externalTransactionID string, status string) (err error) {
	_since := time.Now()
	defer func() {
		result := "ok"
		if err != nil {
		...
		datastoreDurationSummaryVec.WithLabelValues(_d.instanceName, "UpdateTransactionStatus", result).Observe(time.Since(_since).Seconds())
	}()
	return _d.base.UpdateTransactionStatus(externalTransactionID, status)
}
