...
func generateSolanaAddress(ctx context.Context, key, env, pcr2 string, verbose bool) {
	if env != "development" && len(pcr2) != 96 {
		log.Fatal("a valid pcr2 is required to generate and address outside of development\n")
	}
	...
	req, err := http.NewRequest(http.MethodPost, apiBase+"/v1/payments/submit", buf)
	if err != nil {
