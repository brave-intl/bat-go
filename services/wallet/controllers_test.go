...
func (suite *ControllersTestSuite) TestClaimWallet() {
	...
	claimURL := "/v1/wallet/{paymentID}/claim"

	reqFailure, err := http.NewRequest("POST", claimURL, bytes.NewBuffer([]byte("{}")))
	rctx := chi.NewRouteContext()
	rctx.URLParams.Add("paymentID", "non-uuid")
	reqFailure = reqFailure.WithContext(context.WithValue(reqFailure.Context(), chi.RouteCtxKey, rctx))
	...
	reqFailure, err = http.NewRequest("POST", claimURL, bytes.NewBuffer([]byte("{}")))
	rctx = chi.NewRouteContext()
	rctx.URLParams.Add("paymentID", walletID.String())
	reqFailure = reqFailure.WithContext(context.WithValue(reqFailure.Context(), chi.RouteCtxKey, rctx))
	suite.Require().NoError(err, "Failed to create get promotions request")
	rr = httptest.NewRecorder()
	handler.ServeHTTP(rr, reqFailure)
	suite.Assert().Equal(http.StatusNotFound, rr.Code, "did not find wallet")
}
