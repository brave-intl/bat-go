	...
	for i := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.given.reqID != uuid.Nil {
				ds.EXPECT().GetNumActiveCreds(tc.exp.args[0], tc.exp.args[1], tc.exp.args[2], tc.exp.args[4]).Return(0, nil)
			}

			ds.EXPECT().DeleteTimeLimitedV2OrderCredsByOrderTx(tc.exp.args[0], tc.exp.args[1], tc.exp.args[2], tc.exp.args[3]).Return(nil)
