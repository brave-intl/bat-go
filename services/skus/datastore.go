...
type Datastore interface {
	...
	GetNumActiveCreds(ctx context.Context, dbi sqlx.ExtContext, orderID uuid.UUID, now time.Time) (int, error)
...
// GetNumActiveCreds returns the number of creds currently active on an order.
func (pg *Postgres) GetNumActiveCreds(ctx context.Context, dbi sqlx.ExtContext, orderID uuid.UUID, now time.Time) (int, error) {
	...
	var result int
	if err := sqlx.GetContext(ctx, dbi, &result, q, orderID, now); err != nil {
