...
func (pg *Postgres) InsertSignedOrderCredentialsTx(ctx context.Context, tx *sqlx.Tx, soResult *SigningOrderResult) error {
	...
	for i := range soResult.Data {
		...
		next := time.Now().UTC()
		lg.Str("func", "InsertSignedOrderCredentialsTx").Time("start", start).Interface("next", next).Msg("1")
		...
		case timeLimitedV2:
			...
			next = time.Now().UTC()
			lg.Str("func", "InsertSignedOrderCredentialsTx").Time("start", start).Interface("next", next).Msg("2")
			...
			next = time.Now().UTC()
			lg.Str("func", "InsertSignedOrderCredentialsTx").Time("start", start).Interface("next", next).Msg("3")
