...
func TestFilterExpCreds(t *testing.T) {
	...
	type tcExpected struct {
		expCreds []TimeAwareSubIssuedCreds
	}
...

		{
			...
			given: tcGiven{
				creds: []TimeAwareSubIssuedCreds{
					...
					{
						ValidTo: time.Date(2021, time.January, 20, 0, 0, 0, 0, time.UTC),
					},
					{
						ValidTo: time.Date(2022, time.January, 20, 0, 0, 0, 0, time.UTC),
					},
				},
				now: time.Date(2023, time.January, 20, 0, 0, 0, 0, time.UTC),
			},
			exp: tcExpected{
				expCreds: []TimeAwareSubIssuedCreds{
					{
...

		{
			...
			given: tcGiven{
				creds: []TimeAwareSubIssuedCreds{
					...
					{
						ValidTo: time.Date(2023, time.January, 20, 0, 0, 0, 0, time.UTC),
					},
	...
	for i := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			actual := findLastExpCred(tc.given.creds, tc.given.now)
			should.Equal(t, tc.exp.expCreds, actual)
		})
	}
}
