...
var (
	// symbolToUniqueIDMapping - mapping of symbols to unique CoinGecko IDs.
	//
	// This is used to handle symbols that are not unique across different chains.
	// For example, "btc" is used by both Bitcoin and BlackrockTradingCurrency.
	//
	// This mapping should only be used when querying by CoinGecko ID is not possible.
	symbolToUniqueIDMapping = map[string]string{
		"imx":   "immutable-x",
		"abat":  "aave-bat",
		"abusd": "aave-busd",
		"adai":  "aave-dai",
...
func (s *Service) initializeCoingeckoCurrencies(ctx context.Context) (context.Context, error) {
	...
	for _, coin := range *list {
		...
		if uniqueID, ok := symbolToUniqueIDMapping[coin.Symbol]; ok {
			symbolToID[coin.Symbol] = uniqueID
		} else {
