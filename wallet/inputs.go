...
// LinkDepositAccountRequest - the structure for a brave provider wallet creation request
type LinkDepositAccountRequest struct {
	SignedLinkingRequest string `json:"signedLinkingRequest"`
	AnonymousAddress     string `json:"anonymousAddress"`
}

// Validate - implementation of validatable interface
func (ldar *LinkDepositAccountRequest) Validate(ctx context.Context) error {
	var merr = new(errorutils.MultiError)
	if ldar.SignedLinkingRequest == "" {
		merr.Append(errors.New("failed to validate 'signedLinkingRequest': must not be empty"))
	}
	if ldar.AnonymousAddress != "" && !govalidator.IsUUID(ldar.AnonymousAddress) {
...
// Decode - implementation of  decodable interface
func (ldar *LinkDepositAccountRequest) Decode(ctx context.Context, v []byte) error {
	if err := inputs.DecodeJSON(ctx, v, ldar); err != nil {
		return fmt.Errorf("failed to decode json: %w", err)
	}
	return nil
}
...
// HandleErrors - handle any errors from this request
func (ldar *LinkDepositAccountRequest) HandleErrors(err error) *handlers.AppError {
	issues := map[string]string{}
	if errors.Is(err, ErrInvalidJSON) {
		issues["invalidJSON"] = err.Error()
	}
